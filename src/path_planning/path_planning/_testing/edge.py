...
class Edge():
    ...
    def getPartsLengthRatio(self):
        part1Length = math.sqrt((self.coordinate1.x - self.intersection[0]) ** 2 + (
            self.coordinate1.y - self.intersection[1]) ** 2)
        part2Length = math.sqrt((self.intersection[0] - self.coordinate2.x) ** 2 + (
            self.intersection[1] - self.coordinate2.y) ** 2)
