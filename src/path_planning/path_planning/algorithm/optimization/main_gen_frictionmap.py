...
def generate_frictionmap():
    ...
    # set paths
    path2module = os.getcwd() + '/src/path_planning/resource/' #os.path.dirname(os.path.abspath(__file__))
    ...
    reftrack = reftrack_functions.load_reftrack(path2track=path2reftrack_file)
    
    # check whether reference line is closed (is the race track a circuit or not)
    bool_isclosed_refline = reftrack_functions.check_isclosed_refline(refline=reftrack[:, :2])
    
    # calculate coordinates of the track boundaries
    reftrackbound_right, reftrackbound_left = reftrack_functions.calc_trackboundaries(reftrack=reftrack)
    ...
    if bool_show_plots:
        ...
        reftrack_functions.plot_refline(reftrack=reftrack)
    
        # plot spatial grid of friction map
        plot_frictionmap_grid.plot_voronoi_fromVariable(tree=tpa_map,
                                                        refline=reftrack[:, :2],
                                                        trackbound_left=reftrackbound_left,
                                                        trackbound_right=reftrackbound_right)
    
        # plot friction data of friction map
        plot_frictionmap_data.plot_tpamap_fromVariable(tpa_map=tpa_map,
                                                       tpa_data=tpa_data,
                                                       refline=reftrack[:, :2],
                                                       trackbound_left=reftrackbound_left,
                                                       trackbound_right=reftrackbound_right)
